...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    return Math.acos((x1 * x2 + y1 * y2) / (Math.pow(x1 * x1 + y1 * y1, 0.5) * Math.pow(x2 * x2 + y2 * y2, 0.5)));
}
 ...
 */
function isPrime(n) {
    let isPrime = true, count = 1;
    while (++count <= Math.pow(n, 0.5)) {
        isPrime = isPrime && n % count !== 0;
    }
    return isPrime;
}
